    ...
    const confirmPayment = async () => {
      try {
        ...
        const response = await fetch('/api/billing-confirm', {
          ...
          body: JSON.stringify({ orderId, paymentKey, amount, billId, clientId }),
        });
    ...
    return (
      <div style={{ minHeight: '100vh', background: '#F0F7F4', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 20px' }}>
        <div style={{ maxWidth: 400, width: '100%', background: '#fff', borderRadius: 20, padding: '40px 24px', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
          ...
          <p style={{ fontSize: 14, color: '#64748B', lineHeight: 1.6, marginBottom: 24 }}>{errorMessage}</p>
          <button
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F0F7F4', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 20px' }}>
      <div style={{ maxWidth: 400, width: '100%', background: '#fff', borderRadius: 20, padding: '40px 24px', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#0F172A', marginBottom: 12 }}>정산 결제 완료! 🎉</h1>
        <p style={{ fontSize: 14, color: '#64748B', lineHeight: 1.6, marginBottom: 28 }}>
          이번 달 정산이 정상적으로 완료되었습니다.
        </p>
        <button
          onClick={() => router.push('/billing')}
        ...
        <p style={{ fontSize: 12, color: '#94A3B8', marginTop: 16 }}>3초 후 자동으로 이동됩니다...</p>
      </div>
    </div>
  );
}
